...
uint8_t temp_data_line[16000];
volatile uint32_t line_count = 0;
volatile uint32_t lines_captured = 0;
...
void Camera_Init()
{
	...
	TIM2->PSC = 0;
	TIM2->ARR = 1;
	TIM2->CCMR1 = TIM_CCMR1_CC2S_0 | TIM_CCMR1_CC1S_0 | TIM_CCMR1_IC1PSC_0;// | TIM_CCMR1_IC2PSC_0;
	TIM2->CCER = TIM_CCER_CC2E | TIM_CCER_CC1E | TIM_CCER_CC2P;// | TIM_CCER_CC1P;
	...
	WriteRegister(0x11, 0x0B);//REG_CLKRC//0x0B//0x17//0x05
	WriteRegister(0x6B, 0xC0);//REG_DBLV//0xC0

	// Enable scaling
	WriteRegister(0x0C, 0x0C);//REG_COM3//0x08//0x0C

	// Use QCIF output format
	//WriteRegister(0x12, 0x08 );//REG_COM7 | 0x04 for rgb

	//RGB565
	//WriteRegister(0x40, ReadRegister(0x40) | 0x10);
